...
async function callGeminiWithRetry(
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (errorMsg.includes("503") || errorMsg.includes("502") || errorMsg.includes("500") || errorMsg.includes("unavailable") || errorMsg.includes("high demand") || errorMsg.includes("temporarily")) {
        onLogStatic?.(`Gemini đang quá tải, tự chờ và thử lại (${i + 1}/${maxRetries})...`);
        const waitTime = Math.min(Math.pow(2, i) * 1500 + Math.random() * 1000, 15000);
        await delay(waitTime);
        continue;
      }
